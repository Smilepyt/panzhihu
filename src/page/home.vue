<template>
  <div>
    ...
    <inifinite-scroll :scroller="scroller" :loading="loading" @load="loadMore"></inifinite-scroll>
    <!-- 回到顶部组件 -->
...
<script>
  ...
  import inifniteScroll from '../components/inifniteScroll'
  ...
  export default {
    components: {
      "m-swipe": swiper,
      "inifinite-scroll": inifniteScroll,
    ...
    data() {
      return {
        ...
        scroller: null, 
        count: 1 // 计数，与今天相离多少天，用来计算请求日期
      }
    },
    mounted() {
      this.getList(1) // 挂载时，请求数据
    ...
    methods: {
      getList(type) {
        let _this = this
        // type===1 首次请求数据，否则请求过去时间的数据
        if (type) {
          api.getNews().then(data => {
            _this.tops = data.data.top_stories
            _this.list.push(data.data)
          })
        } else {
          api.getNewsByDate(_this.GetDate(_this.count)).then(data => {
            _this.list.push(data.data);
            _this.loading = false;
          });
        }
